...
const TOOLS = [
  ...
  // La descripción de abajo la reescribió una investigación externa (13-ago-2026) que cayó en
  // tres trampas que la propia descripción tendía: (a) recomendaba json_each sobre `procedures`,
  // que el guardián de costos rechaza SIEMPRE (el guardián falla cerrado y no evalúa la
  // selectividad del filtro: eso es diseño, la doc era la equivocada); (b) no decía que
  // a_supplier_critical es una MUESTRA top-5 por proveedor que además incluye nivel high, así
  // que un SUM sobre ella parece la exposición total de un proveedor y no lo es; (c) no
...
const METHODOLOGY = {
  ...
  banderas: {
    ...
    'IT-01': { nombre: 'Plazo Insuficiente para Entregar Ofertas', peso: 8, regla: 'DOS regímenes desde el 12-ago-2026, y el que se aplica depende de los datos del proceso. (A) TÉRMINO LEGAL del Art. 96 del Reglamento, solo en procesos publicados desde el 28-oct-2025 y solo cuando se tienen las dos fechas reales: se cuentan los días hábiles desde el día SIGUIENTE al cierre de respuestas y aclaraciones (COA Art. 158) hasta la fecha límite de entrega de ofertas, descontando feriados (COA Art. 159), y se compara contra la tabla del Registro Oficial 9S 153 pág. 69: más de 10.000 hasta 100.000 → 2 días; hasta 500.000 → 4; hasta 1.000.000 → 6; de 1.000.000 en adelante → 10. Por debajo de 10.000 la tabla no asigna término y el proceso no se evalúa. (B) REFERENCIAL en el resto: días hábiles entre publicación y cierre de ofertas contra 9/13/17 por monto. Esos mínimos NO salen de ninguna norma y el detalle de la bandera lo dice expresamente. Por qué hay dos: la fecha límite para RESPONDER, que es de donde arranca el término legal, NO está en los datos abiertos (la API publica la de PREGUNTAR, que va de 2 a 6 días antes según el Art. 91), y la fecha límite de ofertas viene vacía en el 93% de los procesos. Las dos se obtienen de la ficha pública del SOCE, y el índice se está construyendo, así que la cobertura del régimen (A) crece con el tiempo. En Régimen Especial el portal rotula el hito como "Audiencia de Preguntas y Aclaraciones" y se lo trata como equivalente: es una interpretación y se declara como tal.' },
    ...
    'TR-01': { nombre: 'Información incompleta crítica', peso: 8, regla: 'falta comprador, valor o método; el PROVEEDOR solo se exige desde la adjudicación (status award, contract o complete). Corregido el 5-sep-2026: antes se exigía proveedor también en convocatoria (status tender/active), que por definición no lo tiene aún, y la bandera marcaba como incompletos a todos los procesos en curso (53.459 medidos) cuando lo que medía era "no adjudicado todavía".' },
