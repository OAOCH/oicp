...
const FLAGS = [
  ...
  { code: 'IP-03', category: 'precio', severity: 3, ocp: 'R069', name: 'Modificación Contractual Significativa',
    ...
    logic: '(final_value - award_value) / award_value > 0.15  (pendiente de datos de enmiendas)' },
  { code: 'CC-01', category: 'concentracion', severity: 3, ocp: '', name: 'Proveedor Recurrente en Ínfima Cuantía',
    ...
    logic: 'infima_por_monto(proc) AND infima_count(buyer, supplier, year) >= 5' },
  { code: 'CC-02', category: 'concentracion', severity: 3, ocp: 'R051', name: 'Proveedor Dominante',
    desc: 'Un proveedor concentra más del 40% del gasto de un comprador en un año, en compradores con al menos 10 procesos (el piso evita falsos positivos en compradores muy pequeños). No aplica a catálogo electrónico.',
    legal: 'Principio de concurrencia Art. 6 LOSNCP',
    logic: 'share_of_buyer > 40 AND buyer_total_procs >= 10' },
  { code: 'CC-03', category: 'concentracion', severity: 2, ocp: '', name: 'Proveedor Histórico Permanente',
    desc: 'Un proveedor gana contratos del mismo comprador en 5+ de los últimos 7 años, con un monto total superior a $50,000.',
    legal: 'Patrón de riesgo reconocido por OCP y OECD',
    logic: 'years_active >= 5 AND total_value > 50000' },
  { code: 'CC-04', category: 'concentracion', severity: 2, ocp: 'R070', name: 'Miembro Recurrente de Consorcio',
    desc: 'Un proveedor participa en 2+ procesos-consorcio con el mismo comprador (umbral bajado por la baja frecuencia de consorcios en los datos de SERCOP).',
    legal: 'Art. 25 LOSNCP reformada regula consorcios',
    logic: 'es_consorcio(proc) AND consortium_count(buyer, supplier) >= 2' },
  { code: 'CC-05', category: 'concentracion', severity: 3, ocp: 'R011', name: 'Posible Fraccionamiento',
    ...
    logic: 'infima_count(buyer, supplier) >= 2 AND infima_total_value > infima_threshold' },
